...
def SingleDimension(sorted_data, sort_dim_bin, dim, best_subsetData_list, sample_size, h_y, best_mi, permut, early_stopping, pre_discretized=False, selected_bins=[], num_cutpoints=None, num_selected=None, delta=0.1):
    ...
    for j in range(len(sorted_data)):
        ...
        if early_stopping == 'chi_square' or early_stopping == 'chi_square_adjust':
        ...
        p_current = chi_square(current_value=candidate_mi, previous_value=pre_best_mi, size=size, degree_of_freedom=degree_of_freedom)
        if p_current < p_best and num_pre_bins != len(reformat_all(deepcopy(candidate_subsetData_list), all_classes)):
            p_best = p_current
    ...
    return best_candidate_subsetData_list, best_mi, best_dim, best_value, selected_bin, p_best

class jointDiscretizationPvalue:
    """Main algorithm
    data: data matrix
    ...
    def fit(self, data):
        ...
        num_cutpoints = self.num_cutpoints if self.num_cutpoints else (data.shape[0]-1)* (data.shape[1]-1)
        ...
        while not stop:
            ...
            for each in result:
                if each[-1] < p_best: #or len(dim_list)==1:
                    if self.early_stopping == 'chi_square' and each[-4] in dim_list: #or len(dim_list)==1:
                        continue
                    else:
                        best_candidate_subsetData_list, best_mi, best_dim, best_value, best_selected_bin, p_best = each
            ...
            if self.early_stopping == 'chi_square_adjust':
                ...
                new_delta = self.delta/((n-1)*(p-num_selected-1))

            if p_best <= new_delta and best_candidate_subsetData_list:
...
def optSingleDimension(sorted_data, sort_dim_bin, dim, best_subsetData_list, sample_size, h_y, best_mi, permut, early_stopping, pre_discretized=False, num_cutpoints=None, num_selected=None, delta=0.05, all_classes=[0,1]):
    """This algorithm is for stagewise
    """
    stop, dim_list, value_list, best_mi_list, selected_bins =False, [], [], [], []
    num_pre_bins, p_best = len(best_subsetData_list), np.infty
    while not stop:
        single_best_candidate_subsetData_list, single_best_mi, single_best_dim, single_best_value, selected_bin, p_current = \
...
class stageWiseDiscretizationPvalue:
    ...
    def fit(self, data):
        ...
        h_y = naive_estimate(data[:, -1]) # naive estimate H(Y)
        dims_list, best_values_list, best_mi, best_dim, best_dim_list, step_mi_list, num_selected = [_ for _ in range(len(copy_data[0])-1)], [], 0, None, [], [], 0
        num_cutpoints = self.num_cutpoints if self.num_cutpoints else (n-1)*(p-1)
        num_cutpoints = (data.shape[1]-1) * math.ceil(np.log2(sample_size)) if self.pre_discretized else num_cutpoints
        ...
        while not stop:
            ...
            for each in result:
                if each[-1] < p_best:
                    best_candidate_subsetData_list, best_mi, best_value_list, best_dim, best_mi_sublist, num_selected, p_best = each

            if p_best != 1 and len(reformat_all(best_candidate_subsetData_list, all_classes))!= len(best_subsetData_list_list[0]):
                best_subsetData_list = reformat_all(best_candidate_subsetData_list, all_classes)
                ...
                assert num_selected == len(best_dim_list), "Not same number of cutpoints in Stagewise"
            else:
                stop=True
        pool.close()
